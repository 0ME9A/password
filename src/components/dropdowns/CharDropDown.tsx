...
export default function CharDropDown() {
  ...
  let whiteList = localStorage.getItem("whiteList")
    ? JSON.parse(localStorage.getItem("whiteList") as string)
    : charTypes;
    ...
    <Box>
      <ListItem>
        <ListItemText
          ...
          secondary="Select the symbol types you want to include in your password."
        />
      </ListItem>
      ...
      >
        {charTypes.map((char: string, index: number) => (
          <Button
            ...
            variant={whiteList.includes(char) ? "contained" : "outlined"}
          >
            {char}
          </Button>
        ))}
